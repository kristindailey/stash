...
- **All Items Page + Dashboard Nav Fix** — New `/items` route with `getAllItems` fetcher, Stash logo wraps to `/dashboard`, sidebar active state driven by `usePathname()` across types, collections, and "View all collections".
